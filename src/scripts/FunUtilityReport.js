    ...
    .then((payload) => {
      payload.map((data) => {
        ...
        document.getElementById(
          "uvanls"
        ).innerHTML += `<option value="${TRANS_SNAME}">${TRANS_SNAME}</option>`
      })
    })
}
  ...
  */
		...
		<div class="p-2 p-centered text-center container">
			...
			<div class="columns px-2">
                <div class="column float-left text-tiny ">
					...
					<b>From :</b> ${moment(fromdate).format("LLL")} <br /><b>To :</b> ${moment(
              todate
            ).format("LLL")}</p>
                </div>
  ...
  try {
      ...
      .then((payload) => {
        if (payload === null) {
          ...
          document.getElementById("output2").innerHTML = payload
            .map(
              (
				...
				<td class="text-tiny text-right">${(TRANS_AMT - VAT_AMT).toLocaleString(
                  "en-BD",
                  {
          ...
          document.getElementById("billsummary").innerHTML = `
			...
			<b>Total Payable: </b>${(TRANS_AMT_TOTAL - VAT_AMT_TOTAL).toLocaleString(
            "en-BD",
            {
              maximumFractionDigits: 2,
              style: "currency",
    ...
    document.getElementById("output2").innerHTML += `
		...
		<td class="text-bold text-tiny">${(
        TRANS_AMT_TOTAL - VAT_AMT_TOTAL
